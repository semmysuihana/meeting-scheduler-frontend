...
Meeting Scheduler memiliki backend terpisah yang dibangun menggunakan Node.js, Express, dan TypeScript, dengan PostgreSQL sebagai database utama.

🔗 Backend Repository:
👉 https://github.com/semmysuihana/meeting-scheduler-backend

Backend ini menyediakan seluruh API yang digunakan oleh frontend, termasuk:

Manajemen organizer

Perhitungan slot (timezone-aware)

Validasi meeting (buffer, min-notice, blackout, double-booking)
...
Frontend membutuhkan 1 environment variable penting untuk mengarah ke server backend:

.env
VITE_SERVER_API=http://localhost:4000


Atau jika kamu testing di jaringan LAN / HP hotspot:

VITE_SERVER_API=http://192.168.xx.xx:4000
